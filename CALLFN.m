...
clear all;
load Dic;               % load the dictionary matrix
Dtn = Dic;
...
for i=1:1
    ...
    for it = 1 : iterNum
        ...
        %[F1,P1,R1,ind1] = perfSupp(X_tsbl,indice,'firstlargest', D);
        %[F1,P,R] = perfSupp(X_tsbl, indice, 'largerThan', 1e-5);
        %[F1,P,R] = calc(X_tsbl, indice,Wgen, 'largerThan', 1e-5);
