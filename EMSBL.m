function [X,gamma_ind,gamma_est,count] = EMSBL(Phi, Y, lambda, Learn_Lambda, varargin)
% Sparse Bayesian Learning for Single Measurement Vector (SMV) problems. 
% *** The version is suitable for noisy cases ***
%
% Command Format:
% [X,gamma_ind,gamma_est,count] ...
%     = EMSBL(Phi,Y,lambda,Learn_Lambda,'prune_gamma',1e-4,'max_iters',500,'epsilon',1e-8,'print',0);
% [X,gamma_ind,gamma_est,count] = EMSBL(Phi,Y, lambda, Learn_Lambda, 'prune_gamma', 1e-4);
% [X,gamma_ind,gamma_est,count] = EMSBL(Phi,Y, lambda, Learn_Lambda);
%
% ===== INPUTS =====
...
%
% *** Reference ***
% [1] David P. Wipf, Bhaskar D. Rao, Sparse Bayesian Learning for Basis
% Selection, IEEE Trans. Signal Processing, vol.52, No.8, 2004
%
% *** Author ***
...
while (1)
    ...
    if Learn_Lambda == 1
        ...
        lambda = norm(Y - Phi * mu,'fro')^2/N + lambda * (m-sum(Sigma_w_diag./gamma_old))/N;
    end;
